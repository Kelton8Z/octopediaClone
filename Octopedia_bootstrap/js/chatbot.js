...
//variables for chat token
var chatToken;
var chatID;
var authID="Bearer lnAc0Bu571c.0beYsq4xvN3SvPcUlkyMQ0t1BiD2zVD62gMFodr5lLw";
var watermark="";

//start a conversation with direct line api 3.0
$.ajax({
    type:"POST",
...
function send(text) {
    ...
    $.ajax({
        type: "POST",
        url: "https://directline.botframework.com/v3/directline/conversations/"+chatID+"/activities",
        headers: {
            "Authorization": "Bearer lnAc0Bu571c.0beYsq4xvN3SvPcUlkyMQ0t1BiD2zVD62gMFodr5lLw",
...
//Request message from conversation

function requestMessage(messageID){
    $.ajax({
        type:"GET",
        url:"https://directline.botframework.com/v3/directline/conversations/"+chatID+"/activities?"+watermark,
        headers:{
            "Authorization": authID
        },
        success:function(data){
            ...
            console.log(data.activities[watermark].text);
            newRecievedMessage(data.activities[watermark].text);
        },
        error:function(){
            console.log("error");
        }
